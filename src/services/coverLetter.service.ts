...
`.trim();
  ...
  const guide = PROMPT_GUIDES_BY_MODE[mode][section];
  return guide ? `${common}\n\n${guide}`.trim() : common;
}

const BASIC_PROMPT_GUIDES = {
  GROWTH_PROCESS: `
문항: 성장과정
포인트:
...
분량: 8~12문장 내
`.trim(),
} satisfies Record<CoverLetterSection, string>;

const SENIOR_PROMPT_GUIDES = {
  GROWTH_PROCESS: `
문항: 경력 요약 및 인생관 (시니어)
포인트:
...
const PROMPT_GUIDES_BY_MODE: Record<PromptMode, Record<CoverLetterSection, string>> =
  {
    basic: BASIC_PROMPT_GUIDES,
    senior: SENIOR_PROMPT_GUIDES,
  };
