...
export async function chatCareerStrength(
  ...
  const stored = await getChatState(input.sessionId, CoverLetterSection.CAREER_STRENGTH);
...
`;
  ...
  try {
    ...
    await saveChatState(input.sessionId, CoverLetterSection.CAREER_STRENGTH, {
